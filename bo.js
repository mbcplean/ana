...
let bot;
let ADMIN_ID; // will be set via CLI input
...
function loadBlockedUsers() {
    try {
        ...
        console.error("Error reading blocked users file:", err.message);
    }
    return [];
}
...
function loadUsageData() {
    try {
        ...
        console.error("Error reading usage file:", err.message);
    }
    return {};
}
...
// Utility logging function: logs to console and sends to Telegram user
...
async function main(count, refCode, chatId) {
    // Load usage data to enforce daily limit
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (walletData) {
            ...
            const updatedWallets = existingWallets.concat(walletData);
...
// For admin conversation state
const adminState = {}; // { chatId: { stage: 'idle' | 'block' | 'unblock' } }

// Check if a user is blocked
function isBlocked(chatId, blockedUsers) {
    return blockedUsers.includes(chatId);
}

// Read blocked users at startup
...
rl.question('Enter your Telegram Bot Token: ', (token) => {
    rl.question('Enter your Admin Telegram ID: ', (adminIdInput) => {
        ...
        bot.on('message', async (msg) => {
            ...
            // If the sender is blocked, reply and ignore further processing.
            if (isBlocked(chatId, blockedUsers)) {
                await bot.sendMessage(chatId, 'You are blocked 🚫.');
                return;
            }

            // Admin inline menu handling
            if (chatId === ADMIN_ID && text === '/admin') {
                ...
                await bot.sendMessage(chatId, 'Admin Menu:', adminMenu);
                return;
            }

            // Handle callback queries for admin actions
            // (Using bot.on('callback_query') below)

            // If not admin command, process user conversation flow
            // Initialize state for new users
            if (!userState[chatId]) {
                userState[chatId] = { stage: 'awaiting_count' };
                // Send directions text on start
                const directions = `Welcome!
This bot is for Zoro Airdrop referrals.
Zoro Airdrop link: https://ai.zoro.org?refCode=1Gun5Z8vO3eNBgi
This bot was made by @vikitoshi and @Muhannad2025

Please enter the number of wallets you want to create (max 100 per day):`;
                await bot.sendMessage(chatId, directions);
                return;
            }
            ...
            if (state.stage === 'awaiting_ref') {
                ...
                // Clear user state after starting processing
                delete userState[chatId];
                return;
            }
        });

        // Handle admin callback queries for block/unblock actions
        ...
        // Handle admin text input for block/unblock actions
        bot.on('message', async (msg) => {
            const chatId = msg.chat.id;
            const text = msg.text;
            if (chatId === ADMIN_ID && adminState[chatId] && adminState[chatId].stage !== 'idle') {
                ...
                if (adminState[chatId].stage === 'block') {
                ...
                } else if (adminState[chatId].stage === 'unblock') {
                    if (blockedUsers.includes(targetId)) {
