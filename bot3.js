...
// Helper: Wait for a random delay between min and max milliseconds.
...
// Utility logging function for internal steps (plain text with suffix)
...
async function main(count, refCode, chatId) {
  try {
    ...
    for (let i = 0; i < count; i++) {
      // Wait for a random delay between 1 to 4 seconds before each wallet creation.
      ...
      if (result) {
        ...
        // Combine wallet creation log in one message:
        let header = "✅️ Successful ✅️";
        let body = `Wallet ${i + 1}️⃣ of ${count}️⃣ created`;
        await sendMessageWithHeader(chatId, header, body);
      }
