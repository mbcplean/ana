...
async function main(count, refCode, chatId) {
  try {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const result = await createAndProcessWallet(i + 1, count, refCode, chatId);
      if (result && result.cancelled) break;
      if (result && result.error409) {
        await userLog(chatId, "Error 409 encountered, skipping wallet creation");
        continue;
      }
      if (result) {
        wallets.push(result);
        ...
        // Send a combined wallet creation log (header and body)
