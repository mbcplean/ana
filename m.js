...
async function createAndProcessWallet(walletNumber, totalWallets) {
    ...
    console.log(style("=================================="));
    console.log(style(`Creating wallet ${walletNumber}/${totalWallets}...`));
    const walletData = await createWallet();
    if (!walletData) return null;

    console.log(style(`Wallet created - Address: ${walletData.address}, Username: ${walletData.username}`));
...
async function main(count) {
    ...
    if (fs.existsSync('wallet.json')) {
        try {
            existingWallets = JSON.parse(fs.readFileSync('wallet.json', 'utf8'));
        } catch (error) {
            console.error('Error reading existing wallet.json:', error.message);
        }
    }
    const updatedWallets = existingWallets.concat(wallets);
    fs.writeFileSync('wallet.json', JSON.stringify(updatedWallets, null, 2));
    console.log(chalk.bold.green(`Successfully created and processed ${wallets.length} wallets and saved to wallet.json`));
}
